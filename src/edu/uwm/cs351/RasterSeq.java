...
public class RasterSeq implements Cloneable
{
	...
	private boolean wellFormed() {
		...
		if (data == null) return _report("data is null"); // test the NEGATION of the condition

		// 2. The data array is at least as long as the number of items
		//    claimed by the sequence.
		// #(
		if (data.length < manyItems) return _report("data is too short");
		// #)
		// TODO
		...
		// #(
		if (currentIndex < 0 || currentIndex > manyItems) return _report("currentIndex is out of range");
		// #)
		// TODO
		...
		// #(
		if (hasCurrentElement && currentIndex == manyItems) return _report("currentIndex is not a valid index");
		// #)
	...
	public void advance( )
	{
		...
		// TODO: Implement this code.
		// #(
		if (atEnd()) throw new IllegalStateException("advancing past end");
		if (hasCurrentElement) ++currentIndex;
		hasCurrentElement = currentIndex < manyItems;
		// #)
	...
	public void addAfter(Raster element)
	{
		...
		if (!hasCurrentElement) {
			addBefore(element); // similar behavior
			currentIndex--;
			return;
		}
	...
	public void addAll(RasterSeq addend)
	{
		...
		for (int i=manyItems-1; i>=addIndex; i--)
			data[i+addend.manyItems] = data[i];
		for (int i=0; i < addend.manyItems; ++i) {
			data[addIndex + i] = cloned.data[i];
		}
	...
	private void ensureCapacity(int minimumCapacity)
	{
		...
		Raster[] newData = new Raster[newCap];
